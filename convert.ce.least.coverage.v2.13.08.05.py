...
#this script reads in the .conversion files for drosophilaQhomoS and drosophilaQcelegansS and saves each entry as a dictionary entry with the drosophila gene as the key. Then, it searches the two dictionaries, and adds the hs/ce gene (whichever is lower) to the output file
###functions
##create_conversion_dict(path to .conversion file) - returns a dictionary where the key is the drosophila protein id, the entry is: [subject id, % coverage]
...
dQhS_path = '/Users/ionchannel/research/tools/db/blast/13.proteomes/000.origional.docs/top.hits.coverage.13.08.06/out.topHits.caenorhabditisQ.homoS.coverage'
dQcS_path = '/Users/ionchannel/research/tools/db/blast/13.proteomes/000.origional.docs/top.hits.coverage.13.08.06/out.topHits.caenorhabditisQ.drosophilaS.coverage'


###IOout###
##the main output file
  #frmt: (hsa gene) (dr/ce gene) (% coverage)
conversion_out = open('/Users/ionchannel/research/tools/db/blast/13.proteomes/000.origional.docs/least.coverage.13.08.06/ce.least.coverage.conversion.13.08.06' ,'a')
##the averages file
  #frmt: (dr 1,2,3,4) \n (ce 1,2,3,4)  
averages_out = open('/Users/ionchannel/research/tools/db/blast/13.proteomes/000.origional.docs/least.coverage.13.08.06/ce.least.coverage.conversion.13.08.06.averages' ,'a')
##the distribution file
   #frmt: #drosophila \n [0] [frequency] \n [5] [frequency] ... #celegans ...
distr_out = open('/Users/ionchannel/research/tools/db/blast/13.proteomes/000.origional.docs/least.coverage.13.08.06/ce.least.coverage.conversion.13.08.06.distribution','a') 
no_hits_out = open('/Users/ionchannel/research/tools/db/blast/13.proteomes/000.origional.docs/least.coverage.13.08.06/ce.least.coverage.conversion.13.08.06.noHits' ,'a')
...
###Parse 2###
##create a list of drosophila genes by scanning in the hsa proteome (longest peptide, patch)
hsa_proteome_path = '/Users/ionchannel/13.PROTEOMES.PATH/000.origional.docs/proteome.caenorhabditis.elegans.longest.peptide.fa'
...
print 'Created list of drosophila proteome ids' + '\n'
...
dqhs_dict = create_conversion_dict(dQhS_path)
dqcs_dict = create_conversion_dict(dQcS_path)
...
dr_avg_cov = 0.0
ce_avg_cov = 0.0
dr_avg_cov = calculate_average_coverage_1(dqhs_dict)
ce_avg_cov = calculate_average_coverage_1(dqcs_dict)
##sum of % coverage / total longest peptides patched (use sum_percent_cov)
dr_cov_pep = 0.0
ce_cov_pep = 0.0
dr_cov_pep = sum_percent_cov(dqhs_dict) / 20532.0
ce_cov_pep = sum_percent_cov(dqcs_dict) / 20532.0
##avg_cov * nohit/total
dr_cov_nohit = 0.0
ce_cov_nohit = 0.0

nohit_total = 8887.0 / 20532.0
...
dr_lowest_percent_coverage, dr_name_lowest = find_lowest_coverage(dqhs_dict)
ce_lowest_percent_coverage, ce_name_lowest = find_lowest_coverage(dqcs_dict)
sum_dr = dr_lowest_percent_coverage + dr_avg_cov
sum_ce = ce_lowest_percent_coverage + ce_avg_cov
dr_low_avg = sum_dr / 2
ce_low_avg = sum_ce / 2
#format the output
output = '#format (line 1-Human): average covererage (excludes noHits), sum % coverage / total # of hsa peptides (longest peptide), average coverage * (# of noHits / (# noHits + #hits)), (lowest % coverage + average coverage) / 2, lowest percent coverage (#), lowest percent coverage (name)' + '\n' + '#format (line 2-Drosophila): same as line 1' + '\n'
...
dr_list_cov, dr_list_noround = get_list_cov(dqhs_dict)
ce_list_cov, ce_list_noround = get_list_cov(dqcs_dict)
...
###Parse 4###
##for each proteinid in the drosophila id list, see if it is in the keys of either dictionary. if it is in only one, write the contents of that dictionary entry to file. if it is in two, write the dictionary entry with the lower % coverage. if there is none, write the hsa id to the the no hits file
...
for protein_id in hsa_proteome_id_list:
	...
	if protein_id in dqhs_dict.keys() and protein_id in dqcs_dict.keys():
		dr_entry = []
		ce_entry = []
		dr_entry = dqhs_dict[protein_id]
		ce_entry = dqcs_dict[protein_id]
	...
	#if the protein_id is only in dqhs_dict.keys()
	if protein_id in dqhs_dict.keys() and protein_id not in dqcs_dict.keys():
		dr_entry = []
		dr_entry = dqhs_dict[protein_id]
		...
		#print output
		continue
	#if the protein id is only in dqcs_dict.keys()
	if protein_id in dqcs_dict.keys() and protein_id not in dqhs_dict.keys():
		ce_entry = []
		ce_entry = dqcs_dict[protein_id]
	...
	if protein_id not in dqcs_dict.keys() and protein_id not in dqhs_dict.keys():
		output = protein_id + '\t' + '-' + '\t' + '-' + '\n'
		conversion_out.write(output)
