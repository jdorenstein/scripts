...
IOfasta_in = open ( '/Users/ionchannel/research/db/blast/oct.proteome/000.origional.docs/octProteome.fa', 'r' )
IOblast_in = open ( '/Users/ionchannel/research/db/blast/oct.proteome/octBlast.out', 'r' )
...
#IOpacid_out = open( '/Users/ionchannel/research/scripts/db_checker_6_17/pacid_array.txt', 'a' )
IOError_out = open( '/Users/ionchannel/research/db/blast/oct.proteome/000.origional.docs/errors.script.txt', 'a' )
...
for line in IOblast_in:
	...
	if is_top_hit == 'TRUE': #is_top_hit is being used to only read the top hit
		...
		if lineSplit[0][0:4] == 'lcl|': #this is to create the dictionary entry
			...
			else: #if line != top hit, then add 1 to position and move to next line
				position_in_alignments = position_in_alignments + 1
			if position_in_alignments == 6: #if the position exceedes the number of results, reset counter, exit loop, and create special entry in dictionary
				is_top_hit = 'FALSE'
				blastDict[dict_key] = [ 'null', 100, 6 ]
